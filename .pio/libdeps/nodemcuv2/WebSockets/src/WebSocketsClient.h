...
typedef void (*WebSocketClientEventHandler)(WebSockClient *wsc, WStype_t type, uint8_t * payload, size_t length);
...
class WebSocketsClient : protected WebSockets {
  public:
    ...
    WebSocketsClient(WebSockClient *wscObject);
    ...
    void onEvent(WebSocketClientEventHandler cbEventHandler);
    // void onEventHandler(WebSocketClientEvent cbEvent);
...
#define SSL_FINGERPRINT_NULL NULL
    ...
    WebSocketClientEventHandler _cbEventHandler;
    WebSockClient *_wscObject;
    ...
    virtual void runCbEvent(WStype_t type, uint8_t * payload, size_t length) {
        // Serial.println("runCbEvent()");
        if (_cbEventHandler){
            // Serial.println("Custom Event Handler");
            _cbEventHandler(_wscObject, type, payload, length);
        }
        else if(_cbEvent) {
            // Serial.println("Default Event Handler");
            _cbEvent(type, payload, length);
        }
        else {
            // Serial.println("No Event Handler Founded!");
        }
    }
};
