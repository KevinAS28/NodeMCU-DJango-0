...
WebSocketsClient::WebSocketsClient(WebSockClient* wscObject){
    this->defaultInitialize();
    _wscObject = wscObject;
}
...
void WebSocketsClient::defaultInitialize(){
    _cbEvent             = NULL;
    _client.num          = 0;
...
void WebSocketsClient::loop(void) {
    // Serial.println("WebSocketsClient::loop(void) called");
    if(_port == 0) {
        Serial.println("WebSocketsClient::loop(void): Port is 0");
        return;
    }

    // Serial.println("WebSocketsClient::loop(void) DEBUG 0");
    WEBSOCKETS_YIELD();
    if(!clientIsConnected(&_client)) {
        // do not flood the server
        if((millis() - _lastConnectionFail) < _reconnectInterval) {
            // Serial.println("WebSocketsClient::loop(void): Not connected 0");
            return;
        }

    Serial.println("WebSocketsClient::loop(void) DEBUG 1");
#if defined(HAS_SSL)
        if(_client.isSSL) {
...
#error setCACert not implemented
        ...
        WEBSOCKETS_YIELD();
        // Serial.println("WebSocketsClient::loop(void) DEBUG 3");
#if defined(ESP32)
        if(_client.tcp->connect(_host.c_str(), _port, WEBSOCKETS_TCP_TIMEOUT)) {
#else
        if(_client.tcp->connect(_host.c_str(), _port)) {
#endif
        // Serial.println("WebSocketsClient::loop(void) DEBUG 4");
            connectedCb();
            _lastConnectionFail = 0;
        } else {
            connectFailedCb();
...
void WebSocketsClient::onEvent(WebSocketClientEventHandler cbEventHandler) {
    _cbEventHandler = cbEventHandler;
}
/**
...
void WebSocketsClient::handleClientData(void) {
    if((_client.status == WSC_HEADER || _client.status == WSC_BODY) && _lastHeaderSent + WEBSOCKETS_TCP_TIMEOUT < millis()) {
        ...
        Serial.println("[WS-Client][handleClientData] header response timeout.. disconnecting!\n");
    ...
    if(len > 0) {
        switch(_client.status) {
            ...
            case WSC_BODY: {
                char buf[256] = { 0 };
                _client.tcp->readBytes(&buf[0], std::min(((unsigned int)len), ((unsigned int)sizeof(buf))));
                String bodyLine = buf;
